...
void MSC_MemoryRead (void) {	   //The function MSC_MemoryRead sends data from the onboard RAM to the host
  DWORD n;
  ...
  USB_WriteEP(MSC_EP_IN, (BYTE *)(MSC_MemoryStart + Offset), n);	  //host computer requests new data, http://www.keil.com/support/man/docs/rlarm/rlarm_usb_writeep.htm
  Offset += n;
  Length -= n;
...
void MSC_MemoryWrite (void) {	 // function copies data from the host to the onboard RAM
  DWORD n;
...
void MSC_MemoryVerify (void) {	 //The function checks whether the data written to the onboard RAM is identical to the data sent by the host
  DWORD n;
...
void MSC_Inquiry (void) {	   //Sends the vendor ID, product ID, and product revision number to the host.
